(function () {
  ...
  function jsonldVis(high, jsonld, selector, config) {
    ...
    var h = config.h || 600,
      ...
      minRadius = config.minRadius || 8,
      scalingFactor = config.scalingFactor || 6;
    ...
    function update(source) {
      ...
      nodeEnter
        ...
        .style("stroke", function (d) {
          if (high.includes(d.name)) {
            return "red";
          }

          return d.isIdNode ? "#eb8934" : "#4ECDC4";
        })
        .style("fill", function (d) {
          ...
          if (d.isIdNode) {
            return d._children ? "#eb8934" : "white";
          } else {
      ...
      nodeUpdate
        ...
        .style("stroke", function (d) {
          if (typeof d.parent !== "undefined") {
            if (high.includes(d.parent.name)) {
              return "red";
            }
          }
          if (high.includes(d.name)) {
      ...
      link
        .style("stroke", function (d) {
          if (high.includes(d.source.name)) {
            return "red";
          }
          if (high.includes(d.target.name)) {
            return "red";
          }
        })
        .enter()
    ...
    function computeRadius(d) {
      if (d.children || d._children) {
        if (numEndNodes(d) > 50) {
          return minRadius + numEndNodes(d) / (scalingFactor * 3);
        }
